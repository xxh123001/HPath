...
def main():
    ...
    print("=" * 80)
    print("🚀 生成细粒度阈值组合文件 (intensity 50-60, area 50-60)")
    print("=" * 80)
    print()
    ...
    # Generate files
    print(f"🎯 生成文件...")
    print(f"   Intensity范围: 50-60 (步长1)")
    print(f"   Area范围: 50-60 (步长1)")
    print(f"   预计生成: 11 × 11 = 121 个文件")
    print()
    
    intensity_range = range(50, 61)  # 50-60 inclusive
    area_range = range(50, 61)  # 50-60 inclusive
    
    total = 0
    for intensity_thresh in intensity_range:
        for area_ratio_thresh in area_range:
            ...
            if total % 10 == 0:
                print(f"   进度: {total}/121")
            
            # 显示一些关键组合的统计
            if (intensity_thresh in [50, 55, 60] and area_ratio_thresh in [50, 55, 60]):
                print(f"      intensity{intensity_thresh}_area{area_ratio_thresh}: {positive_count} ROIs ({positive_pct:.1f}%)")
    
    print()
    print(f"✅ 完成！生成了 {total} 个文件")
    print()
    print("=" * 80)
