...
public class MultiChannelOverlayServer extends AbstractTileableImageServer {
    ...
    public BufferedImage getDefaultThumbnail(int z, int t) throws IOException {
        // Use readRegion to ensure proper multi-channel image is created
        var metadata = getMetadata();
        int maxDim = 1024;
        double downsample = Math.max(
            (double)metadata.getWidth() / maxDim,
            (double)metadata.getHeight() / maxDim
        );
