...
import qupath.lib.gui.commands.ProjectHistoryManager;
...
public class CommonActions {
	...
	public CommonActions(QuPathGUI qupath) {
		...
		PROJECT_OPEN = createAction(() -> {
			// Use enhanced dialog with project history  
			java.io.File projectFile = Commands.showEnhancedProjectOpenDialog(qupath);
			if (projectFile != null) {
				try {
					...
					Dialogs.showErrorMessage("Open Project", e.getMessage());
				}
			}
		});
		...
		// Register JSON to GeoJSON handler for drag and drop
