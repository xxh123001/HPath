		...
		if (obj.has("coordinates")) {
			...
			if (factory != null && !Objects.equals(parsingFactory.getPrecisionModel(), factory.getPrecisionModel())) {
				...
				try {
					...
					// Skip precision reduction entirely and keep the high-precision geometry
					// This is safer than trying to force reduction which can corrupt the geometry
					logger.info("Keeping original high-precision geometry to preserve validity");
					// Just ensure it has a valid factory, but don't change precision
					if (geom.getFactory() != parsingFactory) {
						logger.warn("Unexpected factory mismatch, this shouldn't happen");
					}
					// Return the original parsed geometry without precision reduction
					// The higher precision should be fine for most use cases
				}
			}
			return geom;
		}
