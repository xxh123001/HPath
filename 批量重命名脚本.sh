...
# 批量重命名脚本 - 将kt_merged_split目录下的文件重命名为标准格式
# 作者: AI Assistant
# 日期: 2025年10月29日
...
for PROJECT_DIR in "$SOURCE_BASE"/K2024-*; do
    ...
    for SOURCE_FILE in "${!FILE_MAPPING[@]}"; do
        ...
        if [ "$TARGET_FILE" = "kt.json" ]; then
            TARGET_PATH="$PROJECT_DIR/${PROJECT_ID}_kt.json"
        else
            TARGET_PATH="$PROJECT_DIR/${PROJECT_ID}_${TARGET_FILE}"
        fi
        ...
        if [ -f "$TARGET_PATH" ]; then
            log "  ${YELLOW}⊗ 目标文件已存在，跳过: $(basename "$TARGET_PATH")${NC}"
            SKIPPED_FILES=$((SKIPPED_FILES + 1))
            continue
        fi
        ...
        log "    → $(basename "$TARGET_PATH")"
        
        # 执行重命名
        if [ "$DRY_RUN" = false ]; then
            if mv "$SOURCE_PATH" "$TARGET_PATH" 2>/dev/null; then
                PROJECT_RENAMED=$((PROJECT_RENAMED + 1))
    ...
    if [ $PROJECT_ERRORS -gt 0 ]; then
        log "  ${RED}错误: $PROJECT_ERRORS 个文件重命名失败${NC}"
    fi
    log ""
done
